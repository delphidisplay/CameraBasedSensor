# Includes function to determine if BBOX is within ROI. That is its main purpose.

from shapely.geometry import Polygon
import matplotlib.pyplot as plt

def list_of_tuples(nested_list):
    """Creates a list of tuples"""
    return [tuple(l) for l in nested_list]

def list_of_lists(nested_tuple):
    """Creates a list of lists"""
    return [list(l) for l in nested_tuple]
...
def intersection_of_polygons(ROI, BBOX, thresh=0.7, debug=True, showPlot=True, figure="1"):
    ...
    ROI = Polygon(list_of_tuples(ROI)) # create polygon object
    BBOX = Polygon(list_of_tuples(BBOX)) # create polygon object
    ...
    isIntersectCoord = list_of_lists(list(INTERSECT.exterior.coords)) # intersection coordinates
